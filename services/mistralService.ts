...
const extractJson = <T>(text: string): T | null => {
    try {
        ...
        console.error("Failed to parse JSON from Mistral response:", text, error);
        return null;
    }
}
...
const generate = async (apiKey: string, model: string, prompt: string, isJson: boolean = false): Promise<any> => {
    const body: { model: string, messages: { role: string, content: string }[], response_format?: { type: string } } = {
        model,
...
// Mistral doesn't natively support image generation. We prompt a text model to output base64.
...
export const getPromptSuggestions = async (currentPrompt: string, model: string, apiKey: string): Promise<string[] | null> => {
    ...
    const response = await generate(apiKey, model, prompt, true);
...
export const generatePalette = async (prompt: string, model: string, apiKey: string): Promise<string[] | null> => {
    const fullPrompt = `Generate a harmonious 16-color pixel art palette based on this theme: "${prompt}". Your response must be a single JSON object with a key "palette" containing an array of 16 hex color code strings (e.g., "#RRGGBB").`;
    const response = await generate(apiKey, model, fullPrompt, true);
